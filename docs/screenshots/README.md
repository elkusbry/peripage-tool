...
| `ios-share.png` | The iOS Share Extension's editor — same `AdjustmentButtonGroup` pill style as `PreviewView`, with the "Print to Peripage" title and Cancel / Print toolbar buttons. | Captured by mirroring `ShareRootView.editingBody` from inside the host app via a temporary `--show-share-editor` flag, then screenshotting. |
